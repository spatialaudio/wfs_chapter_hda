...
def plot(drive, sel, sec_src, ti=0):
    ...
    sfs.plot2d.loudspeakers(array.x, array.n, sel * array.a,
                            size=0.3, show_numbers=True)
...
Nssd = 2**5  # number of loudspeakers
R = 2  # radius
...
for x_mic in (-1, 0, +1):
    ...
    plt.plot(xr_ref[0, 0, Nssd//2-Nd:Nssd//2+Nd+1],
             xr_ref[0, 1, Nssd//2-Nd:Nssd//2+Nd+1],
             'C7o-', lw=0.1, ms=3, mew=0.75, mfc='w', mec='k')
