...
for i in range(2):
    ...
    ax[i].plot(R*np.cos(phi), R*np.sin(phi), lw=0.25, color='darkgray')
    # non active SSD
    ax[i].plot(x[0, 0, np.squeeze(max_op == 0)],
               x[0, 1, np.squeeze(max_op == 0)],
               'o', color='darkgray', ms=1, mew=0.75,
               mfc='darkgray', mec='darkgray')
    # active SSD
    ax[i].plot(x[0, 0, np.squeeze(max_op > 0)],
               x[0, 1, np.squeeze(max_op > 0)],
...
sfs.plot2d.loudspeakers(array.x, array.n, selection * array.a, size=0.05)
plt.savefig('wfs25d_circSSD_aliasing_sfstoolbox.png')
